...
for($locind=0;$locind -lt ($DomSuppliers);$locind++)
  ...
  1..$Slices | ForEach-Object { $sliceId = 0; } {
    ...
    $job = Start-Job -Name $jobName -ScriptBlock $generateMaterialProductionGraph -ArgumentList $OutputDirectory,$ItemCount,$Transports,$FileNamePrefix,$sliceId,$LocationsArray,$Stores,$ActivityHash,$LocationHash,$SupCanSupply,$LDC1,$LDC2,$IntlSuppliers,$DomSuppliers,$Slices,$StartItemIntl,$EndItemIntl,$StartItemDom,$EndItemDom,$StartAct,$EndAct;
  ...
  1..$Slices | ForEach-Object { $sliceId = 0; } {
    ...
    $job = Start-Job -Name $jobName -ScriptBlock $generateDemand -ArgumentList $OutputDirectory,$StartDate,$Years,$Demands,$Customers,$ItemCount,$FileNamePrefix,$sliceId,$StartItemIntl,$EndItemIntl,$StartItemDom,$EndItemDom,$LocationsArray,$DomSuppliers,$IntlSuppliers,$Stores,$LDC1,$LDC2,$StartStore,$EndStore,$DemandIndexHash;
    if($sliceId -eq ($Slices-2))
	{
  ...
  1..$Slices | ForEach-Object { $sliceId = 0; } {
    ...
    $job = Start-Job -Name $jobName -ScriptBlock $generateCapacityAvailability -ArgumentList $OutputDirectory,$StartDate,$Years,$FileNamePrefix,$sliceId,$LocationsArray,$IntlSuppliers,$DomSuppliers,$Stores,$ResourceHash,$LDC1,$LDC2,$LocationHash,$ResActHash,$FindActKeyHash,$ActivityHash,$FindResKeyHash,$StartRes,$EndRes;
    if($sliceId -eq $Slices-2)
	{
  ...
  1..$Slices | ForEach-Object { $sliceId = 0; } {
	...
	$StoresPerSlice=[math]::floor($Stores/($Slices-4));
	$StoresRem=$Stores%($Slices-4);
	if($sliceId -eq 0)
	{
	$StartLoc=$Stores;
	$EndLoc=($Stores+$LDC1/2);
	}elseif($sliceId -eq 1)
	{
	$StartLoc=($Stores+$LDC1/2);
	$EndLoc=($Stores+$LDC1);
	}elseif($sliceId -eq 2)
	{
	$StartLoc=($Stores+$LDC1);
	$EndLoc=($Stores+$LDC1+$LDC2);
	}
	elseif($sliceId -eq 3)
	{
	$StartLoc=0;
    ...
    $job = Start-Job -Name $jobName -ScriptBlock $generateInventory -ArgumentList $OutputDirectory,$StartDate,$Years,$ItemCount,$Version,$FileNamePrefix,$sliceId,$StartItemIntl,$EndItemIntl,$StartItemDom,$EndItemDom,$LocationsArray,$DomSuppliers,$IntlSuppliers,$Stores,$LDC1,$LDC2,$StartLoc,$EndLoc;
    if($sliceId -ge $Slices-2)
   {
  ...
  1..$Slices | ForEach-Object { $sliceId = 0; } {
    ...
    $job = Start-Job -Name $jobName -ScriptBlock $generateStorageGraph -ArgumentList $OutputDirectory,$Locations,$ItemCount,$Version,$FileNamePrefix,$sliceId,$StartItemIntl,$EndItemIntl,$StartItemDom,$EndItemDom,$LocationsArray,$DomSuppliers,$IntlSuppliers,$Stores,$StartDate,$Years,$LDC1,$LDC2,$StartStore,$EndStore;
    if($sliceId -eq $Slices-2)
	{
	$StartStore+=$StoresPerSlice;
	$EndStore+=$StoresPerSlice+$StoresRem;
...
$generateResourceGroupPriority={
 param($OutputDirectory,$ItemCount,$Transports,$FileNamePrefix,$sliceId,$LocationsArray,$Stores,$LDC1,$LDC2,$ActivityHash,$ResourceHash,$LocationHash,$SupCanSupply,$Slices,$ResActHash,$IntlSuppliers,$DomSuppliers,$StartItemIntl,$StartItemDom,$EndItemIntl,$EndItemDom,$FindActKeyHash,$FindResKeyHash,$FindGroupKeyHash);
	...
	$Groups=172+$IntlSuppliers*$LDC1*($EndItemIntl-$StartItemIntl)+$DomSuppliers*$LDC2*($EndItemDom-$StartItemDom);
	...
	for($ActInd=($IntlSuppliers*$LDC1);$ActInd -lt ($IntlSuppliers*$LDC1)+$Stores+($Stores/10);$ActInd++)
	    ...
	    for($TransInd=0;$TransInd -lt $Transports;$TransInd++)
	        ...
	        for($ItemInd=$StartItemDom;$ItemInd -lt $EndItemDom;$ItemInd++)
			  {
			   $Res=$acty[2+$ItemInd-($StartItemDom)];
		       $reskey=$FindResKeyHash.Get_Item($Res);
			   $MPFile.writeline($ActInd.ToString()+","+$ResKeyMain+","+$TransInd+","+$ItemInd+","+$LocationHash.Get_Item($resloc).ToString()+",0,"+$groupid+",1");
	           $MPFile.writeline($ActInd.ToString()+","+$reskey+","+$TransInd+","+$ItemInd+","+$LocationHash.Get_Item($resloc).ToString()+",0,"+$groupid+",1");
				...
				if($sliceId -eq 0){
                   for($StoreInd=0;$StoreInd -lt $LDC1/2;$StoreInd++){
                     if($storecount -lt 0.3*$totalstores){
					  ...
					  if($storecount -lt 6){
					 $StorageAvail=($EndItemIntl-$StartItemIntl)*4*(6)*200;					
					 ...
					 if($StartDate -ge $AfterRandTime)
					  ...
					  $StorageAvail=($EndItemIntl-$StartItemIntl)*6*200;
					  $randtime=$TimeArr[$StoreInd];
					  [DateTime]$AfterRandTime=$StartDateDup.AddDays($randtime*7);
					  if($StartDate -ge $AfterRandTime)
					  {
                     ...
                     {
                       $StorageAvail=($EndItemDom-$StartItemIntl)*6*200;
                      }
					  else{
					   $StorageAvail=($EndItemDom-$StartItemIntl)*33*200;
					    $randtime=$TimeArr[$StoreInd];
						[DateTime]$AfterRandTime=$StartDateDup.AddDays($randtime*7);
...
$generateStorageGraph={
	param($OutputDirectory,$Locations,$ItemCount,$Version,$FileNamePrefix,$sliceId,$StartItemIntl,$EndItemIntl,$StartItemDom,$EndItemDom,$LocationsArray,$DomSuppliers,$IntlSuppliers,$Stores,$StartDate,$Years,$LDC1,$LDC2,$StartStore,$EndStore);
	$MPFile=[System.IO.StreamWriter] ("$OutputDirectory\$FileNamePrefix-$($sliceId).csv");
...
$generateMaterialNodeParameters={
	param($OutputDirectory,$StartDate,$Years,$ItemCount,$Version,$FileNamePrefix,$sliceId,$SupCanSupply,$LocationsArray,$LDC1,$LDC2,$Stores,$LocationHash,$CommonItems,$DomSuppliers,$IntlSuppliers,$StartItemIntl,$StartItemDom,$EndItemIntl,$EndItemDom,$DemandIndexHash,$StartLoc,$EndLoc);
	$DFile=[System.IO.StreamWriter] ("$OutputDirectory\$FileNamePrefix-$($sliceId).csv");
	$bool=@("Y","N");
	$itemval=0;
	$Dem1=1;
		...
		for($LocInd=0;$LocInd -lt $DomSuppliers;$LocInd++)
       {
		 ...
		 for($locind=$StartLoc;$locind -lt $EndLoc;$locind++)
		 {
		   for($itemind=$StartItemIntl;$itemind -lt $EndItemDom;$itemind++)
		   {
				...
				$DFile.writeline($LocationHash.Get_Item([Int]$location).ToString()+","+$ItemInd+",0,,,,,,,"+$ItemInd%3+",");
...
$generateInventory={
	param($OutputDirectory,$StartDate,$Years,$ItemCount,$Version,$FileNamePrefix,$sliceId,$StartItemIntl,$EndItemIntl,$StartItemDom,$EndItemDom,$LocationsArray,$DomSuppliers,$IntlSuppliers,$Stores,$LDC1,$LDC2,$StartLoc,$EndLoc);
    $DFile=[System.IO.StreamWriter] ("$OutputDirectory\$FileNamePrefix-$($sliceId).csv");
	$bool=@("Y","N");
	$Inv=@(200,250,300);
...
$generateDemand =
{
	...
	param($OutputDirectory,$StartDate,$Years,$Demands,$Customers,$ItemCount,$FileNamePrefix,$sliceId,$StartItemIntl,$EndItemIntl,$StartItemDom,$EndItemDom,$LocationsArray,$DomSuppliers,$IntlSuppliers,$Stores,$LDC1,$LDC2,$StartStore,$EndStore,$DemandIndexHash);
	...
	for($ItemInd=$StartItemIntl;$ItemInd -lt $EndItemDom;$ItemInd++)
	{
	                if($ItemInd % 3 -eq 0)
					 {
					  ...
					  {
					  $DemInd=0;
					  $DemandIndexHash.Set_Item($ItemInd,$DemInd);
					  $Dem1=0;
					  }elseif($Dem1 -eq 0){
					  $DemInd=1;
		              $DemandIndexHash.Set_Item($ItemInd,$DemInd);
	 ...
	 while($StartDate -lt $EndDate)
	{
	  for($LocInd=$StartStore;$LocInd -lt $EndStore+$x;$LocInd++) #replace 5 with $Stores
       {
          ...
          for($ItemInd=$StartItemDom;$ItemInd -lt $EndItemDom;$ItemInd++)
          {
                     ...
                     $DFile.writeline($WeekKey+","+$LocInd+","+$DemandIndexHash.Get_Item($ItemInd)+","+$CustInd+","+$ItemInd+",0,3,3,"+$DemandIndexHash.Get_Item($ItemInd)+","+$DemandQuantity+",,,,,"+$sliceId);
...
\			   
	...
	for($ActInd=($IntlSuppliers*$LDC1);$ActInd -lt ($IntlSuppliers*$LDC1)+$Stores+($Stores/10);$ActInd++)
			...
			for($ActInd=($IntlSuppliers*$LDC1+$Stores+($Stores/10));$ActInd -lt ($IntlSuppliers*$LDC1+$Stores+($Stores/10)+$LDC2*2);$ActInd++)
		{
		    ...
		    for($ItemInd=$StartItemIntl;$ItemInd -lt $EndItemIntl;$ItemInd++)
			{
			  ...
			  if($resind -eq 1 -or $resind -eq 3){
				 ...
				 ##
		       $reskey=$FindResKeyHash.Get_Item($acty[$resind]);
	           $MPFile.writeline($LocationHash.Get_Item([Int]$resloc).ToString()+","+$ActInd+","+"0"+","+$reskey+","+$TransInd+","+$ItemInd+","+$CapCat+",1,1,1");
			   $CapCat="";
			   }
	   ...
	   for($ActInd=($IntlSuppliers*$LDC1+$Stores+($Stores/10)+$LDC2*2);$ActInd -lt ($IntlSuppliers*$LDC1+$Stores+($Stores/10)+$LDC2*2+$DomSuppliers*$LDC2);$ActInd++)
		{
		$items=$EndItemDom-$StartItemDom;
		 for($ItemInd=0;$ItemInd -lt ($items);$ItemInd++)
			{
			  ...
			  for($TransInd=0;$TransInd -lt $Transports;$TransInd++)
			  {
	           ...
	           $MPFile.writeline($LocationHash.Get_Item($resloc).ToString()+","+$ActInd+","+"0"+","+$reskey+","+$TransInd+","+$ItemInd+",O,1,1,1");
			   $reskey=$FindResKeyHash.Get_Item($acty[3+$ItemInd]);
	           $MPFile.writeline($LocationHash.Get_Item($resloc).ToString()+","+$ActInd+","+"0"+","+$reskey+","+$TransInd+","+$ItemInd+",,1,1,1");
